...
# Trabalha a partir da pasta do backend (evita 'cd' no CMD)
WORKDIR /app/backend
...
# Forma shell para que $PORT seja expandido pela variável de ambiente.
CMD gunicorn app:app --bind 0.0.0.0:$PORT --workers 3 --threads 4 --timeout 60
